 ...
 */
function processToken(token: Record<string, unknown> | null) {
 ...
 */
function combineTokens(tokenSources: Array<unknown[] | null | undefined>) {
...
function addMetadata(chainId: string, tokens: Array<Record<string, unknown>>, tokenList: Record<string, { name: string }>) {
...
const swapsStateSelector = (state: { swaps: Record<string, unknown> }) => state.swaps;
...
const selectSwapsControllerState = (state: { engine: { backgroundState: { SwapsController: Record<string, unknown> } } }) =>
  state.engine.backgroundState.SwapsController;

/**
 * Returns the swaps tokens from the state
 */
export const swapsControllerTokens = (state: { engine: { backgroundState: { SwapsController: { tokens: unknown[] } } } }) =>
...
function swapsReducer(state: Record<string, unknown> = initialState, action: { type: string; payload?: unknown }) {
  switch (action.type) {
