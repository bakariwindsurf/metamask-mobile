...
const getAssetType = (selectedAsset: { tokenId?: string; isETH?: boolean } | undefined) => {
  let assetType;
  if (selectedAsset) {
...
const transactionReducer = (state = initialState, action: { type: string; [key: string]: unknown }) => {
  switch (action.type) {
    case REHYDRATE:
      return {
        ...initialState,
...
export const selectTransactionState = (state: { transaction: typeof initialState }) => state.transaction;
